...
public class HttpMixin {
    ...
    private static String onGet(String url) {
        if (PulseConfig.get().httpAllowed == PulseConfig.HttpAllowed.Nothing) return "http://0.0.0.0";
        else if (PulseConfig.get().httpAllowed == PulseConfig.HttpAllowed.NotMeteorApi && url.startsWith("https://meteorclient.com/api")) return "http://0.0.0.0";
        else if (PulseConfig.get().httpAllowed == PulseConfig.HttpAllowed.NotMeteorPing && url.startsWith("https://meteorclient.com/api/online")) return "http://0.0.0.0";
        return url;
    }
